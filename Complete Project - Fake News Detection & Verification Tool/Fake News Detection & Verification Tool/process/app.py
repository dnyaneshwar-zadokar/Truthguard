...
# --- Database Config (Render PostgreSQL fallback to local SQLite) ---
...
def init_database():
    with app.app_context():
        try:
            ...
            admin_email = 'dnyaneshwarzadokar@gmail.com'
            if not User.query.filter_by(email=admin_email).first():
                admin = User(
                    email=admin_email,
                    name='System Administrator',
                    password=generate_password_hash('Dnyaneshwar@123', method='pbkdf2:sha256'),
                    role='admin',
                    is_active=True
                )
